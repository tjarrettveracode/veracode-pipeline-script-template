...
def getapplication(appid):
    appinfo = api.VeracodeAPI().get_app(legacy_id=appid)
...
def getpolicy(appinfo):
    ...
    return api.VeracodeAPI().get_policy(policy_guid)
...
def getdetailedreport(build_id):
    detailedreport = api.VeracodeAPI().get_detailed_report(build_id)
